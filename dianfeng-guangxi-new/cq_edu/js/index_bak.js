...
var subMenuObj = {
	...
	onSelect:function(){
		...
		if(url == "erweima"){ //目前二维码区域不做任何处理
			...
			if(menuBox.position == 0 && this.x == 0 && this.y == 0){
				if(jsonUser == ""){
...
var actObj = {
	...
	show : function(){
		...
		if(!backFlag){
			...
			$("act_btn0").style.background = "images/61_btn1_"+ (this.pos + 1)%2 +".png";
			$("act_btn1").style.background = "images/61_btn2_"+ this.pos +".png";
		}else{
	...
	changeFocus : function(){
		if(!backFlag){
			...
			$("act_btn0").style.background = "url(images/61_btn1_"+ (this.pos + 1)%2 +".png)";
			$("act_btn1").style.background = "url(images/61_btn2_"+ this.pos +".png)";
		}else{
			this.pos = this.pos == 0 ? 1 : 0;
	...
	clickSelect : function(_fn){
		if(this.pos == 0){
			if(!backFlag){
				//var url = "topic/61/index.html?activityID=3";
				//location.href = url;
				this.hide();
				showOrder_2();
			}else{
