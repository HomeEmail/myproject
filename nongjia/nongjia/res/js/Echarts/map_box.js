require.config({
	paths:{
		echarts:'../res/js/Echarts/echarts-2.2.7/build/dist'
	}
});
require(
	[
	...
	function (ec) {
		var myChart = ec.init(document.getElementById('map_box'));
	...
	document.getElementById('map_box').onmousewheel = function (e){
		var event = e || window.event;
		if(zrEvent.getDelta(event) < 0 || curIndx == 0)return;
		curIndx = 0;
		var mt = mapType[curIndx % mapType.length];
		option.tooltip.formatter = '点击选择该地区<br/>当前：{b}';
	...
	myChart.on(ecConfig.EVENT.MAP_SELECTED, function (param){
		...
		if (mt == 'china') {
			...
			option.tooltip.formatter = '点击选择或滚轮上滑切换至全国<br/>当前：{b}';
			option.series[0].mapType = mt;
			myChart.setOption(option, true);
		}else{                           						
			...
			if(!district){
				...
				var city = param.target;
				cityType.push(city);
				mapGeoData.params[city] = {
					getGeoJson: (function (c) {
						var geoJsonName = cityMap[c];
						return function (callback) {
							if(typeof(geoJsonName) == "undefined"){
								setArea(param.target);
								return false;
							}else{				   
								district = true;
								$.getJSON('http://echarts.baidu.com/echarts2/doc/example/geoJson/china-main-city/' + geoJsonName + '.json', callback);
							}
						}
					})(city)
				}
