...
tapOn($('#previewImgBt'),function(e){
	...
	var height=tplBoxSelect.height+90;//90是放文字位置
	//画布宽高
   ...
   // imgLeft=tplBoxSelect['box1'].x;
    ...
    if(boxImg['1'].top>=0){
		...
		ctx.drawImage(starBgImg, 0, 0, tplBoxSelect['box1'].w, tplBoxSelect['box1'].h);//bg
    ...
    ctx.drawImage(crop_canvas_box1, c_left, c_top, imgW, imgH,imgLeft, imgTop, imgW, imgH);
		...
		ctx.drawImage(starMaskImg, 0, 0, tplBoxSelect['box1'].w, tplBoxSelect['box1'].h);//mask
  ...
  //   });
  	...
  	// ctx.fillRect(0,tplBoxSelect['box1'].h-90,tplBoxSelect['box1'].w,90);
  	
  	// ctx.closePath();
   
	addText(ctx,$liuyanName.val(),200,tplBoxSelect['box1'].h+27);
	addText(ctx,$('#liuyanDepartment').val(),200,tplBoxSelect['box1'].h+53);
	addText(ctx,$('#liuyanCity').val(),200,tplBoxSelect['box1'].h+85);
