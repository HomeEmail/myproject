...
$('#tplBox1Action').on('tap',function(e){
	tplBoxTapAction(1);
});
...
tapOn($('#previewOKBt'),function(e){
	...
	var $bt=$(this);
	var bumen=$('#liuyanBumen').val();
	var username=$('#liuyanName').val();
	//上传base64图片数据给服务器
    ...
    /*$.ajax({
	  ...
	  data: { hand: data ,bumen:bumen,username:username},//传递图片、部门和姓名信息给后台
...
tapOn($('#previewImgBt'),function(e){
	...
	var height=tplBoxSelect.height+0;//0是放文字位置
	//画布宽高
   ...
   // imgLeft=tplBoxSelect['box1'].x;
    ...
    if(boxImg['1'].top>=0){
		...
		ctx.drawImage(starBgImg, 0, 0, 497, 445);//bg
    ...
    ctx.drawImage(crop_canvas_box1, c_left, c_top, imgW, imgH,imgLeft, imgTop, imgW, imgH);
		...
		ctx.drawImage(starMaskImg, 0, 0, 497, 445);//mask
  ...
  //   });
	...
	//addText(ctx,$liuyanName.val(),230,445);
